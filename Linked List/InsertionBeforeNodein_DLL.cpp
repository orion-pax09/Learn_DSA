...
Node*InsertionBeforeNode(Node*head , int value){
    Node*previous = head->back;
    Node*newNode = new Node (value , head , previous);
    previous->ptr = newNode;
    head->back = newNode;
    return newNode;
}
int main(){
    ...
    Node*node = InsertionBeforeNode(temp->ptr->ptr->ptr->ptr , 334);
    print(node);
}
